...
// Sub-components
import { ResultView } from './StepReview/ResultView'
import { ReviewView } from './StepReview/ReviewView'
    ...
    const logSessionToDb = async () => {
      ...
      if (step === AppStep.RESULT && ticketCode && !sessionLoggedRef.current) {
        ...
        try {
          ...
          // Get template and background names
          const templateName = selectedTemplate?.name || null
          const backgroundName = selectedBackground || null
  ...
  const handlePrint = async () => {
    if (ticketCode) {
      try {
        setIsPrinting(true)
        await invoke('print_ticket_result', { ticketCode })
        // Mark session as printed in database
        await markSessionPrinted(ticketCode)
        console.log('🖨️ Session marked as printed:', ticketCode)

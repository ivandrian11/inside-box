...
export const Layout: React.FC<LayoutProps> = ({
  ...
  // Determine if timer should be visible based on admin's sessionTimerScope setting
  const getTimerVisible = (): boolean => {
    if (!isSessionActive) return false
    if (step === AppStep.RESULT) return false

    const timerScope = localStorage.getItem('sessionTimerScope') || 'STEP_03_ONLY'

    if (timerScope === 'STEP_03_ONLY') {
      return step === AppStep.CAMERA
    } else if (timerScope === 'STEP_03_04') {
      return step === AppStep.CAMERA || step === AppStep.REVIEW
    } else if (timerScope === 'STEP_03_05') {
      return step === AppStep.CAMERA || step === AppStep.REVIEW || step === AppStep.ARRANGE
    }
  ...
  const isTimerVisible = getTimerVisible()
    ...
    <div className='relative flex flex-col bg-studio-bg w-full h-screen overflow-hidden font-body'>
      ...
      <div className='top-0 left-0 z-40 absolute items-start grid grid-cols-[1fr_auto_1fr] p-6 w-full pointer-events-none'>
        ...
        <div className='flex flex-col items-end gap-4 pointer-events-none'>
          ...
          {/* Timer - Visibility based on admin sessionTimerScope setting */}
          {isTimerVisible && (
            <div

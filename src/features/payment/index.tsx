...
import { DevSimulationButton } from './components/DevSimulationButton'
...
export const StepPayment: React.FC = () => {
  ...
  // Auto-skip payment if debugMode is active
  useEffect(() => {
    if (debugMode) {
      console.log('🐞 [Debug Mode] Auto-skipping payment screen, setting ticket code, starting timer, and skipping to Template Select!')
      setTicketCode(ticketCode)
      startTimer()
      setStep(AppStep.TEMPLATE_SELECT)
    }
  }, [debugMode, ticketCode, setTicketCode, startTimer, setStep])

  // Save ticketCode to global state on mount (only if not in debugMode)
  useEffect(() => {
    if (!debugMode) {
      setTicketCode(ticketCode)
    }
  }, [ticketCode, debugMode, setTicketCode])
  ...
  // Effect to generate QR when QRIS is selected (skip if debugMode is active)
  useEffect(() => {
    if (debugMode) return
